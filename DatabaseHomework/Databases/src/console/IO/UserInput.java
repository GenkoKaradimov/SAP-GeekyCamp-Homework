...
public class UserInput {
	...
	public String student(String[] input) {
		...
		} else if(input[1].equals("CREATE") && input.length == 5) {
	...
	public String course(String[] input) {
		if(input[1].equals("CREATE") && input.length == 5) {
			...
			return context.course.all();
		} else if(input[1].equals("UPDATE")) {
			return context.course.update(input[2], input[3], input[4], input[5]);
		} else if(input[1].equals("DELETE")) {
			return context.course.delete(input[2], input[3]);
		} else if(input[1].equals("STUDENTS")) {
			return context.sc.getCourseStudents(input[2], input[3]);
		} else if(input[1].equals("ADD-STUDENTS")) {
			return context.sc.addStudentToCourse(input[2], Integer.parseInt(input[3]), input[4], input[5]);
		} else if(input[1].equals("REMOVE-STUDENTS")) {
			return context.sc.removeStudentFromCourse(input[2], Integer.parseInt(input[3]), input[4], input[5]);
		}
		
		ConsoleHelp.courseHelp();
		return "";
	}
}
